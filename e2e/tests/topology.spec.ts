...
test.describe('topology projection', () => {
  ...
  test('revision graph diff marks node and edge deltas', async ({ page }) => {
    ...
    // One added line per route: agent.model (flash) and the pinned night
    // window's models[0].model (qwen3.8-max) — values #536/-230.
    ...
    await expect(page.locator('[data-testid="yaml-diff-line"][data-diff="added"]').filter({ hasText: 'litellm/ali/anthropic/qwen3.8-max' })).toHaveCount(1);
