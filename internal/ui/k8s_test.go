...
// Minimal Pipeline CRUD tests (same pattern as Workflow)

func TestK8sClient_GetPipeline(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = v1alpha1.AddToScheme(scheme)
	...
	fakeClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(pl).Build()
	k := NewK8sClient(fakeClient, "test-ns")

	got, err := k.GetPipeline(context.Background(), "test-pipeline")
...
func TestK8sClient_CreatePipeline(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = v1alpha1.AddToScheme(scheme)
	...
	err := k.CreatePipeline(context.Background(), pl)
	if err != nil {
		t.Errorf("unexpected error: %v", err)
		return
	}
	...
	// Verify it was created
	got, err := k.GetPipeline(context.Background(), "new-pipeline")
	if err != nil {
		t.Errorf("pipeline not created: %v", err)
		return
	}
	if got.Name != "new-pipeline" {
		t.Errorf("expected name new-pipeline, got %s", got.Name)
	}
}
